    ...
    try:
        ...
        if not submitted_code.isdigit() or len(submitted_code) != CODE_LENGTH:
            return JSONResponse(
                ...
                content={"detail": f"Invalid code format. Code must be {CODE_LENGTH} digits."},
            )
...
async def login_for_access_token(
    ...
    try:
        ...
        logger.error(f"Fatal error occured during login {e}")
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content={"detail": "An unexpected error occurred during login. Please try again later."},
        )
